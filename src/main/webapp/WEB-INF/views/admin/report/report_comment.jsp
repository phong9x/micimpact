...
<c:url value="/" var="root"></c:url>
		...
		<div class="text-left">
				<div class="" style="display: inline-block; vertical-align; middle;">
					  ...
					  <ul class="nav nav-tabs" >
					    <li  class="active" style="background-color: #f9f9f9;"><a href="${root}admin/report/comment" aria-controls="home" >댓글 신고</a></li>
					    <li  ><a href="${root}admin/report/card" aria-controls="profile" >컨텐츠 신고</a></li>
					  </ul>
			  	</div>
		  </div>
			...
			<tbody>
				<c:forEach items="${list}" var="i">
					<tr>
						...
						<button type="submit" value="${i.commentId}" name="hide" class="btn btn-primary" >숨기기</button>
						</c:if>
						<c:if test="${i.isShow==0 }">
						<button type="submit" value="${i.commentId}" name="show" class="btn  btn-success" >보이기</button>
						</c:if>
	...
	function deleteCard(id) {
		if (confirm("해당 댓글을 삭제하시겠습니까?")) {
			location.href="${root}admin/report/comment?delete="+id;
		}
	}
