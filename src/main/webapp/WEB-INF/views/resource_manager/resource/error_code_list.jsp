	...
	<div class="container">
		...
		<div style="width: 700px; float: left; margin-left: 30px;">
			<div style="margin-bottom: 25px;">
				 <legend class="legende">에러코드 관리
					<div class="search_box">
						<div class="row">
							<select class="form-control" id="options">
								<option value="1" selected="selected">에러코드</option>
								<option value="2">에러메세지</option>
							</select>
							<div class="col-lg-6" >
			...
			<table class="table table-bordered">
				...
				<tbody>
				    <c:forEach items="${list}" var="errors">
						<tr>
							<td>${errors.id}</td>
							<td>${errors.errorCode}</td>
							<td>${errors.errorMassage}</td>
							<td><a class="btn btn-default" href="/res/res_manage/error_code/delete/${errors.id}" >삭제</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
...
function search() {
	...
	if (keyword.length > 0 && option != 0 ){
		var url = "/res/res_manage/error_code/search?option="+ option + "&keyword=" + keyword;
		location.href = url;
	}else {
		location.href = "/res/res_manage/error_code/";
	}
}

function register() {
	location.href = "/res/res_manage/error_code/register"
}
</script>
