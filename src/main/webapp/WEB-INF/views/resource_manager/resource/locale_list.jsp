	...
	<div class="container">
		...
		<div style="width: 700px; float: left; margin-left: 30px;">
			...
			<table class="table table-bordered">
				...
				<tbody>
				    <c:forEach items="${list}" var="locales">
						<tr>
							<td>${locales.id}</td>
							<td>${locales.language}</td>
							<td>${locales.value}</td>
							<td><a class="btn btn-default" href="/res/res_manage/locale/delete/${locales.id}" >삭제</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
...
function search() {
	...
	if (keyword.length > 0 && option != 0 ){
		var url = "/res/res_manage/locale/search?option="+ option + "&keyword=" + keyword;
		location.href = url;
	}else {
		location.href = "/res/res_manage/locale/";
	}
}

function register() {
	location.href = "/res/res_manage/locale/register"
}
</script>
